...
app.get("/createUserTable", (req, res) => {
  let sql =
    "CREATE TABLE users(userId int, name VARCHAR(255), score int, multiplier float, lastMessageNegative bool, lastMessagePositive bool, PRIMARY KEY (userId))";
  db.query(sql, (err) => {
    if (err) {
      throw err;
    }
...
app.get("/createMessageTable", (req, res) => {
  let sql =
    "CREATE TABLE messages(messageId int, userId int, content VARCHAR(10000), rawScoreChange int, effectiveScoreChange int, score int, PRIMARY KEY (messageId))";
  db.query(sql, (err) => {
    if (err) {
      throw err;
    }
...
app.get("/loadUsers", (req, res) => {
  ...
  let sql =
    "INSERT INTO users (userId, name, score, multiplier, messageStreak, rank) VALUES ?";
